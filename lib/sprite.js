...
export class Sprite {
   ...
   */
  updatePosition(game) {
    const simulatedFrameRate = 16;
    const actualFrameRate = game.clock.delta;
    const percentChange = (simulatedFrameRate - actualFrameRate) / simulatedFrameRate * 100;

    let compDx = this.dx;
    if (percentChange > 0) {
      // reduce compDx
      const throttle = compDx * (percentChange / 100);
      compDx -= throttle;
